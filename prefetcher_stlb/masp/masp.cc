...
int64_t masp::vpn_distance(uint64_t current, uint64_t previous)
{
  if (current >= previous)
...
void masp::touch(entry& value) { value.last_used = ++table_timestamp_; }

masp::entry* masp::find_entry(uint64_t pc)
{
...
masp::allocation_result masp::allocate_entry(uint64_t pc)
{
  auto& set = table_.at(static_cast<std::size_t>(pc % TABLE_SETS));
...
void masp::record_trigger(const champsim::modules::stlb_prefetcher_context& context)
{
  if (!context.warmup)
    ++stats_.triggers;
}

void masp::issue_deltas(const champsim::modules::stlb_prefetcher_context& context, const std::vector<int64_t>& deltas)
{
...
void masp::print_common_stats(std::string_view name) const
{
...
void masp::stlb_prefetcher_initialize()
{
  stats_ = {};
  table_ = {};
  table_timestamp_ = 0;
...
void masp::stlb_prefetcher_operate(const champsim::modules::stlb_prefetcher_context& context)
{
  if (context.hit)
    return;
  ...
  if (found->stride_valid)
    candidates.push_back(found->stored_stride);
  candidates.push_back(new_stride);

  issue_deltas(context, candidates);
...
void masp::stlb_prefetcher_final_stats()
{
  print_common_stats("MASP");
  fmt::print("STLB predictor MASP table_lookup {} table_hit {} table_miss {} allocation {} replacement {}\n", table_lookups_, table_hits_,
             table_misses_, table_allocations_, table_replacements_);
}
