...
void stp::record_trigger(const champsim::modules::stlb_prefetcher_context& context)
{
  if (!context.warmup)
    ++stats_.triggers;
}

void stp::issue_deltas(const champsim::modules::stlb_prefetcher_context& context, const std::vector<int64_t>& deltas)
{
...
void stp::print_common_stats(std::string_view name) const
{
...
void stp::stlb_prefetcher_initialize() { stats_ = {}; }

void stp::stlb_prefetcher_operate(const champsim::modules::stlb_prefetcher_context& context)
{
  if (context.hit)
    return;
  record_trigger(context);
  issue_deltas(context, {-2, -1, 1, 2});
}

void stp::stlb_prefetcher_final_stats() { print_common_stats("STP"); }
