...
TRACE_LEVEL_CSV="${OUT_DIR}/sms_trace_level.csv"
BENCHMARK_AGG_CSV="${OUT_DIR}/sms_benchmark_agg.csv"
FIG_PNG="${OUT_DIR}/sms_benchmark_agg.png"
FIG_PDF="${OUT_DIR}/sms_benchmark_agg.pdf"
...
if [ -n "$SELECT_TRACE_JSON" ]; then
    python3 "$TOOL_PY" single-config \
        ...
        --fig-png "$FIG_PNG" \
        --fig-pdf "$FIG_PDF" \
        --figure-title "SMS benchmark-level aggregated metrics" \
        --is-sms
else
    python3 "$TOOL_PY" single-config \
        --result-dir "$RESULT_DIR" \
...
echo "[DONE] SMS fulltrace CSV + figures generated under: $OUT_DIR"
