...
SCRIPT_COMPARE_TAG="$COMPARE_TAG"
SCRIPT_CONFIG_TAG="$CONFIG_TAG"
...
COMPARE_TAG="$SCRIPT_COMPARE_TAG"
CONFIG_TAG="$SCRIPT_CONFIG_TAG"

RUN_N_WARM=${N_WARM:-${DEFAULT_N_WARM}}
RUN_N_SIM=${N_SIM:-${DEFAULT_N_SIM}}
read -r -a DEFAULT_OPTION_ARRAY <<< "${DEFAULT_OPTION:-}"
...
for trace_path in "${TRACE_FILES[@]}"; do
    ...
    echo "[CMD] ${RUN_SCRIPT} ${BINARY_NAME} ${RUN_N_WARM} ${RUN_N_SIM} ${trace_path} ${DEFAULT_OPTION_ARRAY[*]}"
    COMPARE_TAG="$COMPARE_TAG" CONFIG_TAG="$CONFIG_TAG" SKIP_EXISTING="$SKIP_EXISTING" \
        "${RUN_SCRIPT}" "${BINARY_NAME}" "${RUN_N_WARM}" "${RUN_N_SIM}" "${trace_path}" "${DEFAULT_OPTION_ARRAY[@]}" &
