...
DATASET_ORDER = ["spec06", "spec17", "gap", "ligra", "qmm", "parsec"]
...
INFINITE_STLB_COMPARE_FIELDS = [
    "dataset",
    "workload",
    "pref_ipc",
    "infinite_stlb_ipc",
    "infinite_stlb_speedup",
    "infinite_stlb_gain_pct",
    "pref_stlb_mpki",
    "infinite_stlb_mpki",
    "stlb_mpki_reduction",
    "stlb_mpki_reduction_pct",
    "pref_stlb_miss_rate",
    "infinite_stlb_miss_rate",
    "stlb_miss_rate_reduction",
    "stlb_miss_rate_reduction_pct",
]

PERFECT_STLB_COMPARE_FIELDS = [
    "dataset",
    "workload",
    "pref_ipc",
    "infinite_stlb_ipc",
    "perfect_stlb_ipc",
    "infinite_stlb_speedup",
    "perfect_stlb_speedup",
    "infinite_stlb_gain_pct",
    "perfect_stlb_gain_pct",
    "perfect_minus_infinite_speedup",
]
...
def parse_trace_tag_from_result_filename(path: pathlib.Path) -> Optional[str]:
    ...
    for marker in ["-tlb-nopref-", "-tlb-pref-", "-tlb-infinite-stlb-", "-tlb-ideal-demand-", "-tlb-ideal-l1pref-", "-tlb-ideal-all-"]:
        if marker in stem:
...
def compare_infinite_stlb_configs(args: argparse.Namespace) -> None:
    pref = read_workload_csv(pathlib.Path(args.pref_csv))
    infinite_stlb = read_workload_csv(pathlib.Path(args.infinite_stlb_csv))
    common = sorted(set(pref) & set(infinite_stlb), key=lambda k: workload_sort_key({"dataset": k[0], "workload": k[1]}))
    rows: List[Dict[str, object]] = []
    for key in common:
        pref_ipc = to_float(pref[key].get("ipc"))
        infinite_ipc = to_float(infinite_stlb[key].get("ipc"))
        speedup = safe_div(infinite_ipc, pref_ipc)
        pref_stlb_mpki = to_float(pref[key].get("stlb_mpki"))
        infinite_stlb_mpki = to_float(infinite_stlb[key].get("stlb_mpki"))
        stlb_mpki_reduction = pref_stlb_mpki - infinite_stlb_mpki if finite(pref_stlb_mpki) and finite(infinite_stlb_mpki) else math.nan
        stlb_mpki_reduction_pct = safe_div(stlb_mpki_reduction, pref_stlb_mpki)
        pref_stlb_miss_rate = to_float(pref[key].get("stlb_miss_rate"))
        infinite_stlb_miss_rate = to_float(infinite_stlb[key].get("stlb_miss_rate"))
        stlb_miss_rate_reduction = (
            pref_stlb_miss_rate - infinite_stlb_miss_rate if finite(pref_stlb_miss_rate) and finite(infinite_stlb_miss_rate) else math.nan
        )
        stlb_miss_rate_reduction_pct = safe_div(stlb_miss_rate_reduction, pref_stlb_miss_rate)
        rows.append({
            "dataset": key[0],
            "workload": key[1],
            "pref_ipc": pref_ipc,
            "infinite_stlb_ipc": infinite_ipc,
            "infinite_stlb_speedup": speedup,
            "infinite_stlb_gain_pct": (speedup - 1.0) * 100.0 if finite(speedup) else math.nan,
            "pref_stlb_mpki": pref_stlb_mpki,
            "infinite_stlb_mpki": infinite_stlb_mpki,
            "stlb_mpki_reduction": stlb_mpki_reduction,
            "stlb_mpki_reduction_pct": stlb_mpki_reduction_pct * 100.0 if finite(stlb_mpki_reduction_pct) else math.nan,
            "pref_stlb_miss_rate": pref_stlb_miss_rate,
            "infinite_stlb_miss_rate": infinite_stlb_miss_rate,
            "stlb_miss_rate_reduction": stlb_miss_rate_reduction,
            "stlb_miss_rate_reduction_pct": stlb_miss_rate_reduction_pct * 100.0 if finite(stlb_miss_rate_reduction_pct) else math.nan,
        })
    write_csv(pathlib.Path(args.out_csv), INFINITE_STLB_COMPARE_FIELDS, rows)
    if args.fig_png:
        plot_infinite_stlb_compare(rows, pathlib.Path(args.fig_png), pathlib.Path(args.fig_pdf), args.figure_title)
    if args.miss_reduction_fig_png:
        plot_infinite_stlb_miss_reduction(rows, pathlib.Path(args.miss_reduction_fig_png), pathlib.Path(args.miss_reduction_fig_pdf), args.miss_reduction_figure_title)
    log_info(f"infinite STLB compare csv: {args.out_csv}")


def compare_infinite_perfect_stlb_configs(args: argparse.Namespace) -> None:
    pref = read_workload_csv(pathlib.Path(args.pref_csv))
    infinite_stlb = read_workload_csv(pathlib.Path(args.infinite_stlb_csv))
    perfect_stlb = read_workload_csv(pathlib.Path(args.perfect_stlb_csv))
    common = sorted(set(pref) & set(infinite_stlb) & set(perfect_stlb), key=lambda k: workload_sort_key({"dataset": k[0], "workload": k[1]}))
    rows: List[Dict[str, object]] = []
    for key in common:
        pref_ipc = to_float(pref[key].get("ipc"))
        infinite_ipc = to_float(infinite_stlb[key].get("ipc"))
        perfect_ipc = to_float(perfect_stlb[key].get("ipc"))
        infinite_speedup = safe_div(infinite_ipc, pref_ipc)
        perfect_speedup = safe_div(perfect_ipc, pref_ipc)
        rows.append({
            "dataset": key[0],
            "workload": key[1],
            "pref_ipc": pref_ipc,
            "infinite_stlb_ipc": infinite_ipc,
            "perfect_stlb_ipc": perfect_ipc,
            "infinite_stlb_speedup": infinite_speedup,
            "perfect_stlb_speedup": perfect_speedup,
            "infinite_stlb_gain_pct": (infinite_speedup - 1.0) * 100.0 if finite(infinite_speedup) else math.nan,
            "perfect_stlb_gain_pct": (perfect_speedup - 1.0) * 100.0 if finite(perfect_speedup) else math.nan,
            "perfect_minus_infinite_speedup": perfect_speedup - infinite_speedup if finite(perfect_speedup) and finite(infinite_speedup) else math.nan,
        })
    write_csv(pathlib.Path(args.out_csv), PERFECT_STLB_COMPARE_FIELDS, rows)
    if args.fig_png:
        plot_infinite_perfect_stlb_speedup(rows, pathlib.Path(args.fig_png), pathlib.Path(args.fig_pdf), args.figure_title)
    log_info(f"infinite/perfect STLB IPC compare csv: {args.out_csv}")
...
def plot_infinite_stlb_compare(rows: List[Dict[str, object]], fig_png: pathlib.Path, fig_pdf: pathlib.Path, title: str) -> None:
    try:
        import matplotlib
        matplotlib.use("Agg", force=True)
        import matplotlib.pyplot as plt
    ...
    vals = [(float(r["infinite_stlb_speedup"]) - 1.0) * 100.0 for r in rows]
    colors = ["#54a24b" if v >= 0.0 else "#d62728" for v in vals]
    ax.bar(x, vals, width=0.68, color=colors, label="Infinite STLB")
...
def plot_infinite_stlb_miss_reduction(rows: List[Dict[str, object]], fig_png: pathlib.Path, fig_pdf: pathlib.Path, title: str) -> None:
    try:
        import matplotlib
        matplotlib.use("Agg", force=True)
        import matplotlib.pyplot as plt
    ...
    labels = [plot_label(r, "amean") for r in rows]
    x = list(range(len(rows)))
    vals = [float(r["stlb_mpki_reduction_pct"]) for r in rows]
    colors = ["#54a24b" if v >= 0.0 else "#d62728" for v in vals]
    fig, ax = plt.subplots(figsize=(max(12, len(rows) * 0.58), 5.6), dpi=220)
    ax.bar(x, vals, width=0.68, color=colors)
    min_val = min(vals) if vals else 0.0
    max_val = max(vals) if vals else 0.0
    y_min = min(-10.0, math.floor((min_val - 2.0) / 10.0) * 10.0)
    y_max = max(100.0, math.ceil((max_val + 2.0) / 10.0) * 10.0)
    ax.set_ylim(y_min, y_max)
    ax.axhline(0.0, color="black", linewidth=0.9)
    ax.set_title(title)
    ax.set_ylabel("STLB MPKI reduction over pref baseline (%)")
    ax.set_xlabel("Benchmark")
    style_axis(ax)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right")
...
def plot_infinite_perfect_stlb_speedup(rows: List[Dict[str, object]], fig_png: pathlib.Path, fig_pdf: pathlib.Path, title: str) -> None:
    try:
        import matplotlib
        matplotlib.use("Agg", force=True)
        import matplotlib.pyplot as plt
    ...
    x = list(range(len(rows)))
    width = 0.36
    infinite_vals = [(float(r["infinite_stlb_speedup"]) - 1.0) * 100.0 for r in rows]
    perfect_vals = [(float(r["perfect_stlb_speedup"]) - 1.0) * 100.0 for r in rows]
    fig, ax = plt.subplots(figsize=(max(12, len(rows) * 0.62), 5.8), dpi=220)
    ax.bar([i - width / 2 for i in x], infinite_vals, width=width, color="#4c78a8", label="Infinite STLB")
    ax.bar([i + width / 2 for i in x], perfect_vals, width=width, color="#54a24b", label="Perfect STLB")
    all_vals = [v for v in infinite_vals + perfect_vals if finite(v)]
    max_val = max(all_vals) if all_vals else 0.0
    y_max = max(5.0, math.ceil((max_val + 1.0) / 5.0) * 5.0)
    ax.set_ylim(0.0, y_max)
    ax.axhline(0.0, color="black", linewidth=0.9)
    ax.set_title(title)
    ax.set_ylabel("IPC gain over pref baseline (%)")
    ax.set_xlabel("Benchmark")
    style_axis(ax)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right")
    ax.legend(loc="upper left", frameon=True, edgecolor="#b0b0b0")
    fig.tight_layout()
    fig_png.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(fig_png, bbox_inches="tight")
    if fig_pdf:
...
def main() -> None:
    ...
    p_inf = sub.add_parser("compare-infinite-stlb")
    p_inf.add_argument("--pref-csv", required=True)
    p_inf.add_argument("--infinite-stlb-csv", required=True)
    p_inf.add_argument("--out-csv", required=True)
    p_inf.add_argument("--fig-png", default="")
    p_inf.add_argument("--fig-pdf", default="")
    p_inf.add_argument("--figure-title", default="Infinite STLB IPC upper bound")
    p_inf.add_argument("--miss-reduction-fig-png", default="")
    p_inf.add_argument("--miss-reduction-fig-pdf", default="")
    p_inf.add_argument("--miss-reduction-figure-title", default="Infinite STLB STLB-miss reduction")
    p_inf.set_defaults(func=compare_infinite_stlb_configs)

    p_perf = sub.add_parser("compare-infinite-perfect-stlb")
    p_perf.add_argument("--pref-csv", required=True)
    p_perf.add_argument("--infinite-stlb-csv", required=True)
    p_perf.add_argument("--perfect-stlb-csv", required=True)
    p_perf.add_argument("--out-csv", required=True)
    p_perf.add_argument("--fig-png", default="")
    p_perf.add_argument("--fig-pdf", default="")
    p_perf.add_argument("--figure-title", default="Infinite STLB vs perfect STLB IPC speedup")
    p_perf.set_defaults(func=compare_infinite_perfect_stlb_configs)
