...
case "$cmd" in
    ...
    clean-csv)
        ...
        ;;
    *)
        echo "Usage: $0 {all [p]|build-only|run-only [p]|select-json|backend|figures|full-backend|full-trace|backend-all|compare|build-nopref|build-baseline|build-pref|build-infinite|build-infinite-stlb|run-nopref [p]|run-baseline [p]|run-pref [p]|run-infinite [p]|run-infinite-stlb [p]|clean-csv|status}"
        exit 1
        ;;
esac
