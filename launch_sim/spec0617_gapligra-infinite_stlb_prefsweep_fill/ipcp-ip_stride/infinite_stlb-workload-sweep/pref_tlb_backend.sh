...
TRACE_LEVEL_CSV="${OUT_DIR}/infinite_stlb_trace_level.csv"
WORKLOAD_CSV="${OUT_DIR}/infinite_stlb_workload_agg.csv"
FIG_PNG="${OUT_DIR}/infinite_stlb_stlb_miss_causes.png"
FIG_PDF="${OUT_DIR}/infinite_stlb_stlb_miss_causes.pdf"
...
if [ -z "$SELECT_TRACE_JSON" ]; then
    SELECT_TRACE_JSON="${PREFSWEEP_ROOT}/stlb_mpki_gt_1.0_selected_traces_qmm_parsec.json"
fi

FIG_TITLE="infinite STLB selected miss causes"
DONE_LABEL="infinite STLB selected CSV + figures"
if [ "$SELECT_TRACE_JSON" = "ALL" ] || [ "$SELECT_TRACE_JSON" = "FULL" ] || [ "$SELECT_TRACE_JSON" = "NONE" ]; then
    FIG_TITLE="infinite STLB full-trace miss causes"
    DONE_LABEL="infinite STLB full-trace CSV + figures"
fi
