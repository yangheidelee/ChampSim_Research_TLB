...
for trace_dir in "${TRACE_DIR_ARRAY[@]}"; do
    ...
    if [ -n "$TRACE_FILTER" ]; then
        ...
        TRACE_FILES+=("${trace_dir}"/ligra_*.champsimtrace.xz)
        TRACE_FILES+=("${trace_dir}"/ligra_*.champsimtrace.gz)
    fi
done
shopt -u nullglob
...
if [ -n "$SELECT_TRACE_JSON" ] && [ "$SELECT_TRACE_JSON" != "ALL" ] && [ "$SELECT_TRACE_JSON" != "FULL" ] && [ "$SELECT_TRACE_JSON" != "NONE" ]; then
    ...
    mapfile -t SELECTED_TRACE_TAGS < <(python3 - "$SELECT_TRACE_JSON" <<'PY'
import json
import sys

with open(sys.argv[1]) as f:
    payload = json.load(f)
for tag in payload.get("selected_trace_tags", []):
    print(tag)
PY
)
    declare -A SELECTED_TRACE_SET=()
    for tag in "${SELECTED_TRACE_TAGS[@]}"; do
        SELECTED_TRACE_SET["$tag"]=1
    done

    FILTERED_TRACE_FILES=()
    for trace_path in "${TRACE_FILES[@]}"; do
        trace_name=$(basename "$trace_path")
        trace_tag=${trace_name%.champsimtrace.xz}
        trace_tag=${trace_tag%.champsimtrace.gz}
        if [ -n "${SELECTED_TRACE_SET[$trace_tag]:-}" ]; then
            FILTERED_TRACE_FILES+=("$trace_path")
        fi
    done
    TRACE_FILES=("${FILTERED_TRACE_FILES[@]}")
    if [ "${#TRACE_FILES[@]}" -eq 0 ]; then
        echo "[ERROR] No trace files matched selected trace JSON: $SELECT_TRACE_JSON"
        exit 1
    fi
fi

if [ "$DO_BUILD" = "1" ]; then
    echo "[BUILD] nopref config"
...
echo "[INFO] selected trace json=${SELECT_TRACE_JSON:-<none>} matched traces=${#TRACE_FILES[@]}"

running_jobs=0
for trace_path in "${TRACE_FILES[@]}"; do
    trace_name=$(basename "$trace_path")
    echo "[RUN] trace=${trace_name}"
    echo "[CMD] ${RUN_SCRIPT} ${BINARY_NAME} ${RUN_N_WARM} ${RUN_N_SIM} ${trace_path} ${DEFAULT_OPTION}"
    COMPARE_TAG="$COMPARE_TAG" CONFIG_TAG="$CONFIG_TAG" SKIP_EXISTING="$SKIP_EXISTING" \
        "${RUN_SCRIPT}" "${BINARY_NAME}" "${RUN_N_WARM}" "${RUN_N_SIM}" "${trace_path}" "${DEFAULT_OPTION}" &
