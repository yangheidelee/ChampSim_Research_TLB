...
SELECT_TRACE_JSON=${SELECT_TRACE_JSON:-${CHAMPSIM_ROOT}/csv_figure/${BASELINE_SELECT_TAG}/select_trace/data_process_for_compare/stlb_mpki_gt_${SELECT_THRESHOLD}_selected_traces.json}
...
do_run_pref() {
    local p=${1:-$MAX_PARALLEL}
    echo "[STEP] run pref selected traces (parallel=$p)"
    TRACE_DIRS="$TRACE_DIRS" MAX_PARALLEL="$p" SKIP_EXISTING="$SKIP_EXISTING" DO_BUILD=0 N_WARM="$N_WARM" N_SIM="$N_SIM" \
        TRACE_FILTER="$TRACE_FILTER" SELECT_TRACE_JSON="$(select_trace_json)" "$RUN_PREF" "$p" "$TRACE_FILTER"
}

do_select_json() {
    echo "[STEP] reuse selected traces from baseline: $(select_trace_json)"
}
