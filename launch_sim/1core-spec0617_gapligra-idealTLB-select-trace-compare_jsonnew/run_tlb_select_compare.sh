...
do_run_all() {
    ...
    do_run_config "$SELECT_BASE_TAG" "$p"
    do_select_json
    local selected
...
show_status() {
    ...
    echo "NOPREF        : $(result_dir "$SELECT_BASE_TAG")"
...
case "$cmd" in
    ...
    run-baseline|run-pref)
        ...
        do_run_config "$BASELINE_TAG" "${2:-$MAX_PARALLEL}" "$(select_trace_json)"
        ;;
    run-nopref)
        check_env
    ...
    run-demand)
        ...
        do_run_config "$IDEAL_DEMAND_TAG" "${2:-$MAX_PARALLEL}" "$(select_trace_json)"
        ;;
    run-l1pref)
        check_env
        if [ ! -f "$(select_trace_json)" ]; then
        ...
        do_run_config "$IDEAL_L1PREF_TAG" "${2:-$MAX_PARALLEL}" "$(select_trace_json)"
        ;;
    run-allideal)
        check_env
        if [ ! -f "$(select_trace_json)" ]; then
        ...
        do_run_config "$IDEAL_ALL_TAG" "${2:-$MAX_PARALLEL}" "$(select_trace_json)"
        ;;
    status)
        show_status
        ;;
