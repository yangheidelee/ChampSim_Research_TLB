...
def parse_trace_tag_from_result_filename(path: pathlib.Path) -> Optional[str]:
    ...
    for binary_tag in ["tlb-nopref-1core", "tlb-pref-1core", "tlb-ideal-demand-1core", "tlb-ideal-l1pref-1core", "tlb-ideal-all-1core"]:
        marker = f"-{binary_tag}-"
        if marker in stem:
...
def extract_metrics_from_result(path: pathlib.Path) -> Optional[Dict[str, object]]:
    ...
    row: Dict[str, object] = {
        ...
        "llc_access": extract_metric(metric_text, "Core_0_LLC_total_access", 0.0),
        "llc_hit": extract_metric(metric_text, "Core_0_LLC_total_hit", 0.0),
        "llc_miss": extract_metric(metric_text, "Core_0_LLC_total_miss", 0.0),
        "llc_mpki": extract_metric(metric_text, "Core_0_LLC_total_MPKI", math.nan),
        "llc_miss_rate": extract_metric(metric_text, "Core_0_LLC_total_miss_rate", math.nan),
    }
    ...
    if not finite(float(row["llc_mpki"])):
        row["llc_mpki"] = safe_div(float(row["llc_miss"]) * 1000.0, float(row["instructions"]))
    if not finite(float(row["llc_miss_rate"])):
        row["llc_miss_rate"] = safe_div(float(row["llc_miss"]), float(row["llc_access"]))
...
def aggregate_workload(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    total_stlb_access = sum(float(r["stlb_access"]) for r in rows if finite(float(r["stlb_access"])))
    total_stlb_miss = sum(float(r["stlb_miss"]) for r in rows if finite(float(r["stlb_miss"])))
    total_llc_access = sum(float(r["llc_access"]) for r in rows if finite(float(r["llc_access"])))
    total_llc_miss = sum(float(r["llc_miss"]) for r in rows if finite(float(r["llc_miss"])))
    out["stlb_mpki"] = safe_div(total_stlb_miss * 1000.0, total_instr, 0.0)
    out["stlb_miss_rate"] = safe_div(total_stlb_miss, total_stlb_access, 0.0)
    out["llc_mpki"] = safe_div(total_llc_miss * 1000.0, total_instr, 0.0)
    out["llc_miss_rate"] = safe_div(total_llc_miss, total_llc_access, 0.0)
    for key, _, _, _, _ in CAUSES:
        miss_sum = sum(float(r[f"{key}_miss"]) for r in rows if finite(float(r[f"{key}_miss"])))
        out[f"{key}_miss_rate"] = safe_div(miss_sum, total_stlb_access, 0.0)
        out[f"{key}_share"] = safe_div(miss_sum, total_stlb_miss, 0.0)
    return out
...
def select_traces(args: argparse.Namespace) -> None:
    ...
    selected = [row for row in rows if finite(float(row["llc_mpki"])) and float(row["llc_mpki"]) > threshold]
    payload = {
        "source_result_dir": str(pathlib.Path(args.result_dir).resolve()),
        "threshold": {"metric": "Core_0_LLC_total_MPKI", "operator": ">", "value": threshold},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SPEC06+SPEC17+GAP+Ligra TLB select-trace processing")
