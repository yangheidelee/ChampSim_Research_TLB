...
TRACE_LEVEL_CSV="${OUT_DIR}/pref_discard_pgc_trace_level.csv"
WORKLOAD_CSV="${OUT_DIR}/pref_discard_pgc_workload_agg.csv"
FIG_PNG="${OUT_DIR}/pref_discard_pgc_stlb_miss_causes.png"
FIG_PDF="${OUT_DIR}/pref_discard_pgc_stlb_miss_causes.pdf"
...
if [ -z "$SELECT_TRACE_JSON" ]; then
    SELECT_TRACE_JSON="${PREFSWEEP_ROOT}/stlb_mpki_gt_1.0_selected_traces.json"
fi

FIG_TITLE="pref discard PGC selected STLB miss causes"
DONE_LABEL="pref discard PGC selected CSV + figures"
if [ "$SELECT_TRACE_JSON" = "ALL" ] || [ "$SELECT_TRACE_JSON" = "FULL" ] || [ "$SELECT_TRACE_JSON" = "NONE" ]; then
    FIG_TITLE="pref discard PGC full-trace STLB miss causes"
    DONE_LABEL="pref discard PGC full-trace CSV + figures"
fi
