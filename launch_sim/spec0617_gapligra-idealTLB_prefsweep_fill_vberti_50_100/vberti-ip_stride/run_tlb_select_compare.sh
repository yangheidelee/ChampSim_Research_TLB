...
PREF_DISCARD_PGC_TAG="pref-discard-pgc-workload-sweep"
...
check_env() {
    ...
    ensure_exec "$TOOL_PY"
    for tag in "$SELECT_BASE_TAG" "$PREF_DISCARD_PGC_TAG" "$BASELINE_TAG" "$IDEAL_DEMAND_TAG" "$IDEAL_L1PREF_TAG" "$IDEAL_ALL_TAG"; do
...
do_build_all() {
    do_build_config "$PREF_DISCARD_PGC_TAG"
...
do_run_all() {
    ...
    do_run_config "$PREF_DISCARD_PGC_TAG" "$p" "$selected"
...
do_csv_all_for_flow() {
    local select_json="$1"
    do_csv_config "$PREF_DISCARD_PGC_TAG" "pref_discard_pgc" "$select_json" "pref discard PGC STLB miss causes"
...
do_ideal_compare() {
    ...
    python3 "$TOOL_PY" compare-ideal \
        ...
        --baseline-trace-csv "$(trace_csv "$BASELINE_TAG" baseline)" \
        --ideal-demand-trace-csv "$(trace_csv "$IDEAL_DEMAND_TAG" ideal_demand)" \
        --ideal-l1pref-trace-csv "$(trace_csv "$IDEAL_L1PREF_TAG" ideal_l1pref)" \
        --ideal-all-trace-csv "$(trace_csv "$IDEAL_ALL_TAG" ideal_all)" \
        --out-csv "${out_dir}/ideal_stlb_ipc_upperbound_compare.csv" \
        --trace-out-csv "${out_dir}/ideal_stlb_ipc_upperbound_compare_trace_level.csv" \
        ...
        --focus-datasets "gap,xsbench" \
        --focus-out-csv "${out_dir}/ideal_stlb_ipc_upperbound_gap_xsbench_compare.csv" \
        --focus-fig-png "${out_dir}/ideal_stlb_ipc_upperbound_gap_xsbench_compare.png" \
        --focus-fig-pdf "${out_dir}/ideal_stlb_ipc_upperbound_gap_xsbench_compare.pdf" \
        --focus-figure-title "Ideal STLB IPC upper bound: GAP + XSBench"
}

do_discard_baseline_compare() {
    local out_dir
    out_dir="$(flow_root)/data_process_for_compare"
    mkdir -p "$out_dir"
    echo "[STEP] pref_discard_pgc baseline IPC compare (${FLOW_TAG})"
    python3 "$TOOL_PY" compare-discard-baseline \
        --discard-trace-csv "$(trace_csv "$PREF_DISCARD_PGC_TAG" pref_discard_pgc)" \
        --pref-trace-csv "$(trace_csv "$BASELINE_TAG" baseline)" \
        --ideal-demand-trace-csv "$(trace_csv "$IDEAL_DEMAND_TAG" ideal_demand)" \
        --ideal-l1pref-trace-csv "$(trace_csv "$IDEAL_L1PREF_TAG" ideal_l1pref)" \
        --ideal-all-trace-csv "$(trace_csv "$IDEAL_ALL_TAG" ideal_all)" \
        --out-csv "${out_dir}/pref_discard_pgc_baseline_ipc_compare.csv" \
        --trace-out-csv "${out_dir}/pref_discard_pgc_baseline_ipc_compare_trace_level.csv" \
        --fig-png "${out_dir}/pref_discard_pgc_baseline_ipc_compare.png" \
        --fig-pdf "${out_dir}/pref_discard_pgc_baseline_ipc_compare.pdf" \
        --figure-title "IPC gain over pref_discard_pgc baseline" \
        --pref-only-out-csv "${out_dir}/pref_over_pref_discard_pgc_ipc_compare.csv" \
        --pref-only-fig-png "${out_dir}/pref_over_pref_discard_pgc_ipc_compare.png" \
        --pref-only-fig-pdf "${out_dir}/pref_over_pref_discard_pgc_ipc_compare.pdf" \
        --pref-only-figure-title "pref over pref_discard_pgc IPC gain"
}

do_discard_baseline_stlb_reduction_compare() {
    local out_dir
    out_dir="$(flow_root)/data_process_for_compare"
    mkdir -p "$out_dir"
    echo "[STEP] pref_discard_pgc baseline STLB MPKI reduction compare (${FLOW_TAG})"
    python3 "$TOOL_PY" compare-discard-stlb-reduction \
        --discard-trace-csv "$(trace_csv "$PREF_DISCARD_PGC_TAG" pref_discard_pgc)" \
        --pref-trace-csv "$(trace_csv "$BASELINE_TAG" baseline)" \
        --ideal-demand-trace-csv "$(trace_csv "$IDEAL_DEMAND_TAG" ideal_demand)" \
        --ideal-l1pref-trace-csv "$(trace_csv "$IDEAL_L1PREF_TAG" ideal_l1pref)" \
        --ideal-all-trace-csv "$(trace_csv "$IDEAL_ALL_TAG" ideal_all)" \
        --out-csv "${out_dir}/pref_discard_pgc_baseline_stlb_mpki_reduction.csv" \
        --trace-out-csv "${out_dir}/pref_discard_pgc_baseline_stlb_mpki_reduction_trace_level.csv" \
        --fig-png "${out_dir}/pref_discard_pgc_baseline_stlb_mpki_reduction.png" \
        --fig-pdf "${out_dir}/pref_discard_pgc_baseline_stlb_mpki_reduction.pdf" \
        --figure-title "STLB MPKI reduction over pref_discard_pgc baseline"
}
...
show_status() {
    ...
    echo "PREF_DISCARD  : $(result_dir "$PREF_DISCARD_PGC_TAG")"
...
case "$cmd" in
    ...
    compare)
        ...
        do_discard_baseline_stlb_reduction_compare
        ;;
    compare-discard|discard-compare)
        check_env
        do_discard_baseline_compare
        do_discard_baseline_stlb_reduction_compare
        ;;
    compare-discard-stlb|discard-stlb-compare)
        check_env
        do_discard_baseline_stlb_reduction_compare
        ;;
    build-baseline|build-pref)
        ...
        do_build_config "$BASELINE_TAG"
        ;;
    build-discard|build-pref-discard-pgc)
        check_env
        do_build_config "$PREF_DISCARD_PGC_TAG"
        ;;
    build-nopref)
    ...
    run-baseline|run-pref)
        ...
        ;;
    run-discard|run-pref-discard-pgc)
        check_env
        do_run_config "$PREF_DISCARD_PGC_TAG" "${2:-$MAX_PARALLEL}" "$(require_select_trace_json)"
        ;;
    run-nopref)
        check_env
    ...
    clean-csv)
        ...
        ;;
    *)
        echo "Usage: $0 {all [p]|build-only|run-only [p]|select-json|backend|figures|full-backend|full-trace|backend-all|compare|compare-discard|discard-compare|compare-discard-stlb|discard-stlb-compare|build-nopref|build-baseline|build-pref|build-discard|build-pref-discard-pgc|build-demand|build-l1pref|build-allideal|run-nopref [p]|run-baseline [p]|run-pref [p]|run-discard [p]|run-pref-discard-pgc [p]|run-demand [p]|run-l1pref [p]|run-allideal [p]|clean-csv|status}"
        exit 1
        ;;
esac
