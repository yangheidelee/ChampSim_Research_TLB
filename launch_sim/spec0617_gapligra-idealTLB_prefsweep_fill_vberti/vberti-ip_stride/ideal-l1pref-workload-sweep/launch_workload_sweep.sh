...
compare_tag_from_dir() {
    ...
    if [ "$parent" = "spec0617_gapligra-idealTLB_prefsweep_fill_vberti" ]; then
        echo "${parent}/${name}"
    elif [ "$grandparent" = "spec0617_gapligra-idealTLB_prefsweep_fill_vberti" ]; then
...
TRACE_DIRS="${TRACE_DIRS:-/data0/tzh/champsim_traces/SPEC06:/data0/tzh/champsim_traces/SPEC17:/data2/zcq/gap_dpc:/data0/tzh/champsim_traces/Ligra:/data0/tzh/champsim_traces/QMM:/data0/tzh/champsim_traces/PARSEC:/data2/zcq/champsim_traces_xsbench}"
TRACE_FILTER=${2:-${TRACE_FILTER:-${BENCH_FILTER:-}}}
SELECT_TRACE_JSON=${SELECT_TRACE_JSON:-}
if [ -z "$SELECT_TRACE_JSON" ]; then
    PREFSWEEP_ROOT=$(dirname "$(dirname "$SCRIPT_DIR")")
    DEFAULT_SELECT_TRACE_JSON="${PREFSWEEP_ROOT}/stlb_mpki_gt_1.0_selected_traces.json"
    if [ -f "$DEFAULT_SELECT_TRACE_JSON" ]; then
        SELECT_TRACE_JSON="$DEFAULT_SELECT_TRACE_JSON"
    fi
fi
...
if [ "${#TRACE_FILES[@]}" -eq 0 ]; then
    echo "[ERROR] No SPEC06/SPEC17/GAP/Ligra/QMM/PARSEC/XSBench trace files found in: $TRACE_DIRS"
