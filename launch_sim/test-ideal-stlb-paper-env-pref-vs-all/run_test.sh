...
BINARY_NAMES=(tlb-paperenv-pref-1core tlb-paperenv-ideal-all-1core)
IDEAL_MODES=(off all)
...
def trace_from_log(path):
    name = path.name
    for marker in [
        "-tlb-paperenv-pref-1core.log",
        "-tlb-paperenv-ideal-all-1core.log",
    ]:
...
trace_csv = out_dir / "paper_env_pref_vs_ideal_all_trace_compare.csv"
...
summary_csv = out_dir / "paper_env_pref_vs_ideal_all_summary_compare.csv"
