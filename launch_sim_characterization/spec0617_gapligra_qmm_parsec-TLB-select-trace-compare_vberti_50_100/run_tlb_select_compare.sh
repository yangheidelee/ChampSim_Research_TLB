...
usage() {
    cat <<EOF
Usage: ./run_tlb_select_compare.sh [all|build-only|run-only|run-nopref|run-permit|run-pref|run-discard|select-json|backend|postprocess|status|clean-csv] [MAX_PARALLEL]
