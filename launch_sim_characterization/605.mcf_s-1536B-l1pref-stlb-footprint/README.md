# SPEC17 mcf L1D/STLB Footprint Characterization

这个 case 固定跑 SPEC17 `605.mcf_s-1536B` trace，用两个 ChampSim JSON 配置生成两个 bin：
...
```text
/data0/tzh/champsim_traces/SPEC17/605.mcf_s-1536B.champsimtrace.xz
```
...
`discard_pgc` 配置使用 `vberti_nocross`，禁止跨页预取；已有 `l1pref` 结果作为 Permit PGC。对比表输出到 `csv_figure/pgc_compare/`。

## 运行

```bash
cd /home/zcq/git_prj/ChampSim/launch_sim_characterization/605.mcf_s-1536B-l1pref-stlb-footprint
...
只补跑 Discard PGC 并生成 Permit PGC vs Discard PGC 对比表：

```bash
N_WARM=20 N_SIM=50 ./scripts/run_discard_pgc.sh
```
