...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot L1D demand virtual-page access patterns.")
    parser.add_argument("--input", required=True, type=Path, help="Input l1d_vpn_trace.csv")
    parser.add_argument("--outdir", required=True, type=Path, help="Output directory")
    parser.add_argument("--window", default=10000, type=int, help="Window size in accesses")
...
def save_line_or_scatter(x, y, ylabel, title, output, marker_size=0.2):
    fig, ax = plt.subplots(figsize=(12, 4.8))
    if len(x) <= 200000:
        ax.plot(x, y, linewidth=0.35)
    else:
        ax.scatter(x, y, s=marker_size, rasterized=True)
    ax.set_xlabel("L1D demand access id")
...
def save_symlog_scatter(x, y, ylabel, title, output, marker_size=0.15):
    fig, ax = plt.subplots(figsize=(12, 4.8))
    if len(x) <= 200000:
        ax.plot(x, y, linewidth=0.35)
    else:
