# 433.milc-127B Demand Data TLB Pattern (Non-random PPN Allocation)

本目录用于分析 `433.milc-127B` 中 real demand data load 的三条 TLB 序列：L1 DTLB access、实际执行 STLB tag lookup 的 STLB access，以及其中 lookup 结果为 miss 的 STLB miss。一次记录同时保存 VA/VPN 和翻译返回的真实 PA/PPN，指令翻译与 data prefetch translation 均不进入这些序列。

本目录是独立的非随机物理页分配实验，配置使用 `virtual_memory.randomization=false`。ChampSim 不再 shuffle 空闲 PPN 列表，而是按实际首次分配顺序从空闲列表取页。它用于与默认 `randomization=1` 结果做 physical-page-placement sensitivity 对比；由于物理地址会影响 cache/DRAM 映射，两套结果属于不同系统配置。

目录初始复制自随机配置，因而复制来的旧 `result/` 与 `csv_figure/` 不能代表非随机结果。脚本通过 `result/configuration.env` 校验配置；在完成一次非随机模拟前，不允许直接后处理旧结果。
...
SKIP_EXISTING=0 ./launch_sim_characterization/433.milc-127B-demand-data-tlb-pattern-nonrandom/scripts/run_all.sh all
```
...
./launch_sim_characterization/433.milc-127B-demand-data-tlb-pattern-nonrandom/scripts/run_all.sh smoke
```

只重新后处理：

```bash
./launch_sim_characterization/433.milc-127B-demand-data-tlb-pattern-nonrandom/scripts/run_all.sh analyze
```
