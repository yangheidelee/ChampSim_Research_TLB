...
CONFIG_MARKER="${CASE_DIR}/result/configuration.env"

if [ ! -s "$CONFIG_MARKER" ] || ! grep -q '^VM_RANDOMIZATION=false$' "$CONFIG_MARKER"; then
    echo "[ERROR] Missing completed non-random VM result marker: $CONFIG_MARKER" >&2
    echo "[HINT] Run scripts/run_all.sh all to replace copied randomized results." >&2
    exit 1
fi
