...
BINARY_NAME="433.milc-127B-demand-tlb-pattern-nonrandom-1core"
...
CONFIG_MARKER="${RESULT_DIR}/configuration.env"
...
is_complete() {
    ...
    [ -s "$CONFIG_MARKER" ] || return 1
    grep -q '^VM_RANDOMIZATION=false$' "$CONFIG_MARKER" || return 1
...
printf 'VM_RANDOMIZATION=running\n' > "$CONFIG_MARKER"
"$BINARY" \
    ...
    "$TRACE" \
    > "$LOG_FILE"
printf 'VM_RANDOMIZATION=false\nBINARY_NAME=%s\nTRACE=%s\n' "$BINARY_NAME" "$TRACE" > "$CONFIG_MARKER"
