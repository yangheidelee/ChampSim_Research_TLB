...
if [ "$#" -lt 2 ]; then
    echo "Usage: ./script/run_config.sh CONFIG_NAME BINARY_NAME [MAX_PARALLEL] [SIMULATOR_OPTIONS...]" >&2
    exit 1
fi
...
echo "[INFO] config=${CONFIG_NAME} binary=${BINARY_NAME} simulator_options=${EXTRA_SIM_ARGS[*]:-(none)}"
