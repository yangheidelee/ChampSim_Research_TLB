...
CONFIGS = {
    ...
    "translation_only": {"label": "Translation Only", "short": "translation_only", "binary": "pgc-translation-only-1core", "result_dir": CASE_DIR / "result" / "translation_only"},
}
CAUSES = [
...
def ipc_rows(raw_by_config: dict[str, dict[str, dict[str, float]]], tags: list[str]) -> tuple[list[dict[str, object]], list[dict[str, object]], list[dict[str, object]]]:
    ...
    by_dataset: dict[str, list[dict[str, object]]] = defaultdict(list)
    for trace_tag in tags:
        if not all(trace_tag in raw_by_config[c] for c in ("nopref", "permit_pgc", "discard_pgc")):
            continue
        dataset, workload = trace_info_from_tag(trace_tag)
...
def write_pairwise_outputs(rows_by_config: dict[str, list[dict[str, object]]]) -> None:
    ...
    pairs = [
        ...
        ("translation_only_vs_discard", "discard_pgc", "translation_only", "Translation Only vs Discard PGC"),
        ("permit_vs_translation_only", "translation_only", "permit_pgc", "Permit PGC vs Translation Only"),
    ]
