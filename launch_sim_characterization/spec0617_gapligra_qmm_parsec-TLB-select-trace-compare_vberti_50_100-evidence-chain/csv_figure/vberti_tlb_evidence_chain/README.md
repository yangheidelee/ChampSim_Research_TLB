...
本目录的分析范围为 **全部数据集**，并使用副本中已经完成的 `nopref`、`discard_pgc` 和 `permit_pgc` 日志进行后处理，不重新运行模拟器，也不读取或依赖 translation-only 日志。共纳入 **179** 条三配置齐全的 trace。
...
2. `02_cross_page_flow_funnel.pdf`：从跨页 request 一直追到 STLB useful。全体 trace 按计数求和后，PQ drop 为 **89.35%**；只有 **0.248%** 的跨页请求触发 STLB translation，最终只有 **0.02522%** 成为 STLB useful。即使只看已经通过 PQ 的候选，也只有 **2.329%** 触发 translation、**0.2368%** 最终 useful。
3. `03_prediction_quality_and_coverage.pdf`：验证“发得多”是否等于“覆盖多”。STLB useful 仅相当于 discard demand miss 的 **0.1867%**，而 permit 相对 discard 的净 demand miss reduction 为 **1.483%**。
...
- 跨页候选到 STLB translation 的保留率：**0.248017%**。
- 跨页候选到 STLB useful 的保留率：**0.0252162%**。
- 已通过 PQ 的候选到 STLB translation/useful：**2.32926% / 0.236819%**。因此 PQ 是显著损失，但不能单独解释剩余候选的低有效覆盖。
- useful 相对于 discard demand miss：**0.186683%**。
- permit 相对 discard 的净 demand STLB miss reduction：**1.48314%**。
...
- GAP and mcf workload摘要：gap:sssp: useful/miss=0.0124%, net=0.268%, gap:tc: useful/miss=0.0168%, net=0.359%, gap:pr: useful/miss=0.0266%, net=0.154%, gap:bc: useful/miss=0.0462%, net=0.395%, gap:cc: useful/miss=0.133%, net=3.75%, spec06:429.mcf: useful/miss=0.194%, net=0.873%, gap:bfs: useful/miss=0.23%, net=3.08%, spec17:605.mcf_s: useful/miss=0.253%, net=4.3%

与 demand miss reduction 绝对相关性较高的几个 trace-level 指标为：

- `cross_requested_mpki`: rho=0.795, p=2.67e-40, n=179
- `useful_vs_discard_miss_pct`: rho=0.720, p=7.06e-30, n=179
- `pollution_per_fill_pct`: rho=-0.641, p=1.68e-21, n=174
- `cross_pq_drop_rate_pct`: rho=0.583, p=1.1e-17, n=179
...
```bash
python3 script/postprocess_evidence_chain.py
```
